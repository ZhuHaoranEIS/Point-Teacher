...
class RFLA_FCOSHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        # g.wrprint("labels:", labels)itelines(["当前图片的GT_label为:\n", str(gt_labels), '\n'])
        # g.writelines(["当前图片的GT_bbox为:\n", str(gt_bboxes), '\n'])
        # g.writelines(["标签分配后的label为：\n", str(labels), '\n'])
        # g.writelines(["标签分配后的bbox_targets为：\n", str(bbox_targets), '\n'])
        ...
        # g.writelines(["flatten_points:\n", str(flatten_points), '\n'])
        # FG cat_id: [0, num_classes -1], BG cat_id: num_classes
        bg_class_ind = self.num_classes
        pos_inds = ((flatten_labels >= 0)
                    & (flatten_labels < bg_class_ind)).nonzero().reshape(-1)

        # print("flatten_cls_scores:", flatten_cls_scores)
        # print("flatten_cls_scores size:", flatten_cls_scores.shape)
        # print('----------------------------------------------')
        # print("flatten_labels:", flatten_labels)
        # print("flatten_labels size:", flatten_labels.shape)
