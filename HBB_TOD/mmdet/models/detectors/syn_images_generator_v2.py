...
from .data_augument_bank import obb2poly_le90
...
def strong_augmentation(img, gt_points, gt_labels, pseudo_points, pseudo_labels, pseudo_bboxes):
    B, C, H, W = img.shape
    img_aug_list = []
    aug_gt_points_list = []
    aug_gt_labels_list = []
    ...
    for i in range(B):
        ...
        aug_pseudo_bboxes = pseudo_bboxes[i].clone()
        ...
        if len(aug_pseudo_bboxes) != 0:
            w = (aug_pseudo_bboxes[:,0] - aug_pseudo_bboxes[:,2]).abs().reshape(-1,1)
            h = (aug_pseudo_bboxes[:,1] - aug_pseudo_bboxes[:,3]).abs().reshape(-1,1)
            x = torch.min(aug_pseudo_bboxes[:,[0,2]], dim=1)[0].reshape(-1,1)
            y = torch.min(aug_pseudo_bboxes[:,[1,3]], dim=1)[0].reshape(-1,1)
            aug_pseudo_bboxes = torch.cat([x+w/2,y+h/2,w,h], dim=1)
            aug_pseudo_bboxes = bbox_cxcywh_to_xyxy(aug_pseudo_bboxes)
...
def MIL_gen_proposals_from_cfg(pseudo_points, pseudo_boxes, fine_proposal_cfg, gt_boxes, img_meta):
    gen_model = fine_proposal_cfg['gen_mode']
    if gen_model == 'refine':
        proposals_list, proposals_valid_list = fine_proposals_from_cfg(pseudo_boxes, fine_proposal_cfg, img_meta=img_meta)
    ...
    for i in range(len(pseudo_boxes)):
        proposals_reference_list.append(pseudo_boxes[i].unsqueeze(1).repeat(1, num_aug, 1).reshape(-1, 4))
        proposals_real_list.append(gt_boxes[i].unsqueeze(1).repeat(1, num_aug, 1).reshape(-1, 4))
...
def gen_negative_proposals(gt_points, proposal_cfg, aug_generate_proposals, img_meta):
    ...
    for i in range(len(gt_points)):
        ...
        x2 = x1 + torch.rand(num_neg_gen) * 100
        y2 = y1 + torch.rand(num_neg_gen) * 100
...
def generate_black_paper(img, bb_occupied, img_syn, pattern, prior_size,
    ...
    img_syn[poly_area_inds[:,0],poly_area_inds[:,1], poly_area_inds[:,2]] = 255
