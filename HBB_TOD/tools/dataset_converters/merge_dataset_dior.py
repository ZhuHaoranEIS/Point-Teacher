...
def merge_coco_files(folder_path):
    ...
    dataset_all = [dior, dior,
                   ...
                   dotav2, dotav2,
                   xview, xview]
    img_save_path = '/data/zhr/spc_vehicle_dior/images/'
        ...
        for idx in tqdm(range(data_len)):
            ...
            if (save_or_not == True) and (filename not in img_name_all):
                ...
                if train_or_val[0] == 0:
                    ...
                    new_image_path = os.path.join('/data/zhr/spc_vehicle_dior/image/train', filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle_dior/image/val', filename)
                # save image
                # ori_image_path = os.path.join(image_path, filename)
                ...
                if train_or_val[0] == 0:
                    new_data_infos_train.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle_dior/image/train', new_filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle_dior/image/val', new_filename)
                shutil.copy(ori_image_path, new_image_path)
    ...
    new_vehicle_train_dict['info']['data_created'] = '2024.3.12'
    ...
    anno_prefix = '/data/zhr/spc_vehicle_dior/annotations/'
    mkdir(anno_prefix)
