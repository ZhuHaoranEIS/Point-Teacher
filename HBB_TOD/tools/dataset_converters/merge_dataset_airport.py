...
def merge_coco_files(folder_path):
    ...
    dataset_all = [dior, dior,
                   ...
                   dotav2, dotav2,
                   xview, xview]
    img_save_path = '/data/zhr/spc_airport/images/'
        ...
        for idx in tqdm(range(data_len)):
            ...
            for j, bbox in enumerate(bboxes):
                ...
                if cate in id_temp:
                    ...
                    new_category_name.append('airport')
            ...
            if (save_or_not == True) and (filename not in img_name_all):
                ...
                if train_or_val[0] == 0:
                    ...
                    new_image_path = os.path.join('/data/zhr/spc_airport/image/train', filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_airport/image/val', filename)
                # save image
                ...
                if train_or_val[0] == 0:
                    new_data_infos_train.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_airport/image/train', new_filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_airport/image/val', new_filename)
                shutil.copy(ori_image_path, new_image_path)
    ...
    temp_out = '/home/zhr/mmdet-rfla/tools/dataset_converters/new_temp.json'
    mmcv.dump(new_data_infos_train, temp_out_train)
    mmcv.dump(new_data_infos_val, temp_out_val)
    mmcv.dump(new_data_infos, temp_out)
    new_vehicle_train = mmcv.load(temp_out_train)
    new_vehicle_val = mmcv.load(temp_out_val)
    new_vehicle = mmcv.load(temp_out)

    new_vehicle_train_dict = dict()
    new_vehicle_train_dict['categories'] = [{"id": 0, "name": "airport", "supercategory": "mark"}]
    new_vehicle_val_dict = dict()
    new_vehicle_val_dict['categories'] = [{"id": 0, "name": "airport", "supercategory": "mark"}]
    new_vehicle_dict = dict()
    new_vehicle_dict['categories'] = [{"id": 0, "name": "airport", "supercategory": "mark"}]
    '''
    info{
    "year" : int,                # 年份
    "version" : str,             # 版本
    ...
    new_vehicle_train_dict['info']['description'] = "This is a airport dataset"
    new_vehicle_train_dict['info']['contributor'] = 'Zhu Haoran'
    new_vehicle_train_dict['info']['url'] = 'null'
    new_vehicle_train_dict['info']['data_created'] = '2024.3.5'
    new_vehicle_train_dict['info']['datasets'] = 'DIOR'
    ...
    anno_prefix = '/data/zhr/spc_airport/annotations/'
    mkdir(anno_prefix)
    ...
    new_vehicle_val_dict['info']['description'] = "This is a airport dataset"
    new_vehicle_val_dict['info']['contributor'] = 'Zhu Haoran'
    new_vehicle_val_dict['info']['url'] = 'null'
    new_vehicle_val_dict['info']['data_created'] = '2024.3.5'
    new_vehicle_val_dict['info']['datasets'] = 'DIOR'
    ...
    new_vehicle_dict['info'] = {}
    new_vehicle_dict['info']['year'] = 2024
    new_vehicle_dict['info']['version'] = '1.0'
    new_vehicle_dict['info']['description'] = "This is a airport dataset"
    new_vehicle_dict['info']['contributor'] = 'Zhu Haoran'
    new_vehicle_dict['info']['url'] = 'null'
    new_vehicle_dict['info']['data_created'] = '2024.3.5'
    new_vehicle_dict['info']['datasets'] = 'DIOR'
    new_vehicle_dict['annotations'] = []  ####需添加
    new_vehicle_dict['images'] = []   ##3需添加
    targetidssp1 = 0
    for j in range(0, len(new_vehicle)):
        Anns = new_vehicle[j]['ann']
        segmentations = []
        areas = []
        image_item = dict()
        image_item['file_name'] = new_vehicle[j]['file_name']
        image_item['height'] = new_vehicle[j]['height']
        image_item['width'] = new_vehicle[j]['width']
        image_item['id'] = new_vehicle[j]['id']
        new_vehicle_dict['images'].append(image_item)
        for k in range(0, len(Anns['labels'])):
            annotation_item = dict()
            annotation_item['image_id'] = new_vehicle[j]['id']
            annotation_item['bbox'] = [round(Anns['bboxes'][k][0], 1), round(Anns['bboxes'][k][1], 1), round(Anns['bboxes'][k][2], 1), round(Anns['bboxes'][k][3], 1)]
            annotation_item['area'] = round(Anns['bboxes'][k][2] * Anns['bboxes'][k][3], 1)
            annotation_item['segmentation'] = [[Anns['bboxes'][k][0], Anns['bboxes'][k][1], Anns['bboxes'][k][0] + Anns['bboxes'][k][2], Anns['bboxes'][k][1] + Anns['bboxes'][k][3]]]
            annotation_item['category_id'] = Anns['labels'][k]
            ...
            annotation_item['id'] = targetidssp1 # 从0开始
            targetidssp1 = targetidssp1 + 1
            new_vehicle_dict['annotations'].append(annotation_item)
    anno_prefix = '/data/zhr/spc_airport/annotations/'
    mkdir(anno_prefix)
    out_ann_file = anno_prefix + 'trainval.json'
    print("当前数据集共有", targetidssp1, '个标签')
    print('save to {}'.format(out_ann_file))
    json.dump(new_vehicle_dict, open(out_ann_file, 'w'), indent = 4) # indent=4 更加美观显示 ##########################
