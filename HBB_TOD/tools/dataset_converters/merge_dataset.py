...
def merge_coco_files(folder_path):
    ...
    json_all = ['/data/zhr/DIOR/trainval.json', '/data/zhr/DIOR/test.json', 
                ...
                '/data/zhr/xview/v1/annotations/xview_train_v1.json', '/data/zhr/xview/v1/annotations/xview_val_v1.json']
    images_all = ['/data/xc/DIOR/JPEGImages-trainval', '/data/xc/DIOR/JPEGImages-test',
                  ...
                  '/data/zhr/xview/v1/train/image', '/data/zhr/xview/v1/val/image']
    format_all = ['jpg', 'jpg',
                  'png', 'png',
                  'png', 'png',
                  'png', 'png']
    ...
    dataset_all = [dior, dior,
                   ...
                   dotav2, dotav2,
                   xview, xview]
    img_save_path = '/data/zhr/spc_vehicle/images/'
        ...
        for idx in tqdm(range(data_len)):
            ...
            if (save_or_not == True) and (filename not in img_name_all):
                ...
                if train_or_val[0] == 0:
                    ...
                    new_image_path = os.path.join('/data/zhr/spc_vehicle/image/train', filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle/image/val', filename)
                # save image
                # ori_image_path = os.path.join(image_path, filename)
                ...
                if train_or_val[0] == 0:
                    new_data_infos_train.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle/image/train', new_filename)
                else:
                    new_data_infos_val.append(data_infos_temp)
                    ori_image_path = os.path.join(image_path, filename)
                    new_image_path = os.path.join('/data/zhr/spc_vehicle/image/val', new_filename)
                shutil.copy(ori_image_path, new_image_path)
    ...
    new_vehicle_train_dict['info']['data_created'] = '2024.1.28'
    new_vehicle_train_dict['info']['datasets'] = 'DIOR/AI-TOD-v2/DOTA-v2/XVIEW'
    ...
    anno_prefix = '/data/zhr/spc_vehicle/annotations/'
    mkdir(anno_prefix)
