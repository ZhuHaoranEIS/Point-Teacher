...
class SODAADOTADataset(CustomDataset):
    ...
    def __init__(self,
        ...
        super(SODAADOTADataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def _results2submission(self, id_list, dets_list, out_folder=None):
        ...
        files = [
            osp.join(out_folder, 'Task1_' + cls + '.json')
        ...
        for img_id, dets_per_cls in zip(id_list, dets_list):
            for f, dets in zip(file_objs, dets_per_cls):
                ...
                for bbox in bboxes:
                    json_element = [img_id, str(bbox[-1])
                                   ] + [f'{p:.2f}' for p in bbox[:-1]]
                    f.writelines(' '.join(json_element) + '\n')
