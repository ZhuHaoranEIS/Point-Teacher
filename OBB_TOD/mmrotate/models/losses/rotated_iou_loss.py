...
def DN_iou_loss(pred, target, hyper=0.2, linear=False, mode='log', eps=1e-6):
    ...
    if linear:
        ...
        warnings.warn(
            ...
            'poly_iou_loss is deprecated, please use "mode=`linear`" '
            'instead.')
    base_loss = rotated_iou_loss_test(pred, target, linear, mode, eps)
    anx = hyper / 2
    w = target[:,2]
    h = target[:,3]
    loss_banck = []
    for i in [-1, 0 ,1]:
        for j in [-1, 0, 1]:
            ...
            targets[:,2] = targets[:,2] - anx * w * i
            targets[:,3] = targets[:,3] - anx * h * j
            loss_banck.append((rotated_iou_loss_test(pred, targets, linear, mode, eps)).reshape(-1,1))
