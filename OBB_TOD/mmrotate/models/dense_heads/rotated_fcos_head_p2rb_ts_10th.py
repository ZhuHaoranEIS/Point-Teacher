...
class TS_P2RBRotatedFCOSHead(RotatedAnchorFreeHead):
    ...
    def get_pseudo_bbox(self,
        ...
        results = self.gnerate_pseudo(all_level_points, gt_points, gt_labels, gt_bboxes, img_metas, img_list, 
    ...
    def loss_pseudo(self,
        ...
        return loss_cls, loss_bbox, loss_centerness, cls_scores_per_gt, pos_inds
    ...
    def loss(self,
        ...
        return loss_bbox, loss_centerness, pos_inds
    ...
    def gnerate_pseudo(self, points, gt_points_list, gt_labels_list, gt_bboxes_list, img_metas, img_list, 
                    flatten_cls_scores, flatten_bbox_preds, flatten_centernesses):
        num_levels = len(points)
        # the number of points per img, per lvl
        num_points = [center.size(0) for center in points]
    ...
    def _gnerate_pseudo_single(self, gt_points, gt_labels, gt_bboxes, cls_scores, bbox_preds, centernesses, 
                            img_metas, img_list, points, num_points_per_lvl):
        num_points = points.size(0)
        ...
        # inds = assign_result.gt_inds
        # pos_inds = (inds != 0).nonzero(as_tuple=False).reshape(-1)    
        # img = img_list.permute(1, 2, 0).contiguous().cpu().numpy()
        # import cv2
        ...
        # coordinates_flip = bbox_preds[pos_inds,:].cpu()
        ...
        #                 labels=np.ones(len(gt_bboxes)).astype(int),
        #                 bbox_color='green',
        #                 text_color='green',
        #                 out_file='/home/zhuhaoran/SODA/mmrotate/models/detectors/vis_points/P2RB_sys.png')
