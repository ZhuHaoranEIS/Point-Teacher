...
class PSLA_RotatedFCOSHead(RotatedAnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 train_cfg=dict(
                     assigner=dict(
                         ...
                         iou_cost=dict(type='RIoUCost', iou_mode='iou', weight=2.0))),
                 init_cfg=dict(
                     type='Normal',
                     layer='Conv2d',
                     std=0.01,
    ...
    def loss(self,
        ...
        flatten_bbox_preds_img = [
            self.strides[i]*(bbox_preds[i].reshape(num_imgs, -1, 4))
            for i in range(len(bbox_preds))
        ]
        flatten_angle_preds_img = [
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list, img_metas, 
                    flatten_cls_scores, flatten_bbox_preds, flatten_angle_preds):
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, cls_scores, bbox_preds, angle_preds, img_metas, points, 
                           num_points_per_lvl):
        ...
        # ltrb -> cx cy wh
        bbox_preds_refine = torch.cat([bbox_preds, angle_preds], dim=1).clone().detach()
        bbox_preds_refine = self.preds_decoder.decode(points_ori, bbox_preds_refine)
        cls_pred_refine = cls_scores.clone().detach()
        
        assign_result = self.assigner.assign(bbox_preds_refine, cls_pred_refine, gt_ori, gt_labels, gt_bboxes_ignore=None)
        inds = assign_result.gt_inds #num_points
        inds = inds - 1

        inds_mask = inds[...,None].repeat(1, num_gts).to(bbox_targets.device) #num_points, num_gts
        point_mask = torch.arange(num_gts).repeat(num_points, 1).to(bbox_targets.device) # num_points, num_gts
        ...
        GT_j = (points[pos_inds,:]).reshape(-1,2)
        # import pdb; pdb.set_trace()
        # imagename = batch_img_metas[0]['img_path']
        image_path = img_metas['filename']
        image = cv2.imread(image_path)
        for i in range(len(gt_labels)):
            cx, cy ,w1, h1, theta =  gt_ori[i].tolist()
            x1 = cx - w1/2
            x2 = cx + w1/2
            y1 = cy - h1/2
            y2 = cy + h1/2
        ...
        for i in range(GT_j.shape[0]):
            # import pdb; pdb.set_trace()
            center_x, center_y = GT_j[i,:]
            color = PALETTENOISE[1]
            radius = 2
            cv2.circle(image, (int(center_x), int(center_y)), radius, color, 2)
        image_store_path = "/home/zhuhaoran/SODA/mmrotate/models/dense_heads/assginment_vis/"
