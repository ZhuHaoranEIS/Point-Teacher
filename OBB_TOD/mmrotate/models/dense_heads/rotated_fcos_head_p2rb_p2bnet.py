...
class P2B_P2RBRotatedFCOSHead(RotatedAnchorFreeHead):
    ...
    def forward_single(self, x, scale, stride, points):
        ...
        return cls_score, points_ori, bbox_pred_dxdy, bbox_pred_theta, bbox_pred_dwdh, bbox_pred_decode, objectness
    ...
    def loss(self,
        ...
        labels, rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, bbox_preds_dwdh_list, \
            gt_bboxes_insider_list, pos_gts, labels_pseudo, pseudo_hbbox_xyxy, inds, assign_result = results
        ...
        return loss_cls, pred_rbbox_iou, rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, bbox_preds_dwdh_list, \
               pos_gts, loss_objectness, flatten_labels_pseudo, pseudo_hbbox_xyxy, inds, assign_result
    
    @force_fp32(
        apply_to=('all_level_points', 'bbox_preds_dxdy', 'bbox_preds_theta',
                  'bbox_preds_dwdh', 'bbox_preds_cxcywhtheta'))
    def loss_aug(self,
                all_level_points,
                bbox_preds_dxdy,
                ...
                augument_type=None, 
                augument_factor=None):
        assert len(bbox_preds_dxdy) == len(bbox_preds_cxcywhtheta)
        num_imgs = bbox_preds_dxdy[0].size(0)
        ...
        results = self.get_targets_aug(all_level_points, gt_points, gt_labels, gt_bboxes_ref, insider, img_list, img_metas, 
                                                            flatten_bbox_preds_dxdy_img, flatten_bbox_preds_theta_img,
                                                            flatten_bbox_preds_dwdh_img, flatten_bbox_preds_cxcywhtheta_img,
                                                            assign_result)
        
        rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, bbox_preds_dwdh_list, \
        _, pos_gts = results
        

        return rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, bbox_preds_dwdh_list, pos_gts
    ...
    def _loss_bbox(self,
        ...
        assert len(cls_scores) == len(bbox_preds_cxcywhtheta)
        num_imgs = cls_scores[0].size(0)
        bbox_targets = self.get_regression(all_level_points, gt_points, gt_labels, gt_bboxes_ref, insider, img_list, img_metas, 
                                        pseudo_hbbox_xyxy, inds)
        ...
        if len(pos_inds) > 0:
            ...
            pos_bbox_preds = bbox_cxcywh_to_xyxy(pos_bbox_preds[:,:4])
            pos_decoded_target_decode = distance2bbox(pos_points, pos_bbox_targets) # xyxy

            loss_bbox = self.loss_dxdydwdhda(pos_bbox_preds,
                                             pos_decoded_target_decode,
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list, gt_bboxes_ref_list, insider_list, img_list, img_metas, 
        ...
        gt_bboxes_insider_list, pos_gts, labels_pseudo_list, pseudo_hbbox_xyxy, inds, assign_result = results
        ...
        return concat_lvl_labels, rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, \
               bbox_preds_dwdh_list, gt_bboxes_insider_list, pos_gts, \
               concat_lvl_labels_pseudo, pseudo_hbbox_xyxy, inds, assign_result
               
    def get_regression(self, all_level_points, gt_points, gt_labels, gt_bboxes_ref, insider, img_list, img_metas, 
                                        pseudo_hbbox_xyxy, inds):
        ...
        # get labels and bbox_targets of each image
        bbox_targets_list, _, _, _ = multi_apply(
                self._get_regression_single,
                gt_points, 
                gt_labels, 
                gt_bboxes_ref,
                insider,
        ...
        return concat_lvl_bbox_targets
    
    def get_targets_aug(self, points, gt_bboxes_list, gt_labels_list, gt_bboxes_ref_list, insider_list, img_list, img_metas, 
                    flatten_bbox_preds_dxdy, flatten_bbox_preds_theta, flatten_bbox_preds_dwdh, 
                    flatten_bbox_preds_cxcywhtheta, assign_result):
        """Compute regression, classification and centerness targets for points
        in multiple images.
        ...
        for i in range(flatten_bbox_preds_dxdy[0].shape[0]):
            temp_dxdy = []
            temp_theta = []
            temp_dwdh = []
            temp_cxcywhtheta = []
        ...
        rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, bbox_preds_dwdh_list, \
        gt_bboxes_insider_list, pos_gts = results

        torch.cuda.empty_cache()
        
        return rbbox_preds_list, bbox_preds_dxdy_list, bbox_preds_theta_list, \
               bbox_preds_dwdh_list, gt_bboxes_insider_list, pos_gts
    ...
    def _get_targets_single(self, gt_bboxes, gt_labels, gt_bboxes_ref, insider_list, cls_scores, bbox_preds_dxdy, bbox_preds_theta,
        ...
        if inverse_indices.size(0) != 0:
            ...
            max_x = grouped_points[:,:,0].max(dim=0)[0].reshape(-1,1)
            max_y = grouped_points[:,:,1].max(dim=0)[0].reshape(-1,1)
            del grouped_points
            
            grouped_points_min = 2*max(H,W)*torch.ones(inverse_indices.size(0), num_gts, 2, device=filtered_mask_points.device, dtype=filtered_mask_points.dtype)
            grouped_points_min[range(inverse_indices.size(0)), inverse_indices, :] = filtered_mask_points
            min_x = grouped_points_min[:,:,0].min(dim=0)[0].reshape(-1,1)
            min_y = grouped_points_min[:,:,1].min(dim=0)[0].reshape(-1,1)
            del grouped_points_min

            pseudo_hbbox_xyxy = torch.cat([min_x, min_y, max_x, max_y], dim=1)
        else:
            pseudo_hbbox_cxcy = 2 * self.strides[0] * torch.ones(num_gts, 4, device=bbox_preds_dxdy.device, dtype=bbox_preds_dxdy.dtype)
            pseudo_hbbox_cxcy[:,:2] = gt_ori
            pseudo_hbbox_xyxy = bbox_cxcywh_to_xyxy(pseudo_hbbox_cxcy)
            del pseudo_hbbox_cxcy
        ...
        return labels, rbbox_preds_average, bbox_preds_dxdy_average, bbox_preds_theta_average, \
            bbox_preds_dwdh_average, gt_bboxes_ref, pos_gts, labels_pseudo, pseudo_hbbox_xyxy, inds, assign_result
    
    def _get_targets_aug_single(self, gt_bboxes, gt_labels, gt_bboxes_ref, insider_list, bbox_preds_dxdy, bbox_preds_theta,
                            bbox_preds_dwdh, bbox_preds_cxcywhtheta, assign_result, img_list, img_metas, points):
        # image_name = img_metas['ori_filename']
        num_points = points.size(0)
        if insider_list is not None:
            insider_inds = (insider_list == 0).nonzero(as_tuple=False).reshape(-1)
        ...
        return rbbox_preds_average, bbox_preds_dxdy_average, bbox_preds_theta_average, \
            bbox_preds_dwdh_average, gt_bboxes_ref, pos_gts
    
    
    def _get_regression_single(self, gt_points, gt_labels, gt_bboxes_ref, 
                               insider_list, pseudo_boxes, inds, img_list, img_metas, points, num_points_per_lvl):
        num_points = points.size(0)
        if insider_list is not None:
            insider_inds = (insider_list == 0).nonzero(as_tuple=False).reshape(-1)
            gt_labels = gt_labels[insider_inds]
            pseudo_boxes = pseudo_boxes[insider_inds]
        num_gts = gt_labels.size(0)
        if num_gts == 0:
            return gt_labels.new_full((num_points,), self.num_classes)

        ### Generate bbox target
        pseudo_boxes = pseudo_boxes[None].expand(num_points, num_gts, 4)
        xs, ys = points[:, 0], points[:, 1]
        xs = xs[:, None].expand(num_points, num_gts)
        ys = ys[:, None].expand(num_points, num_gts)

        left = xs - pseudo_boxes[..., 0] #numpoints, num_gt
        right = pseudo_boxes[..., 2] - xs
        top = ys - pseudo_boxes[..., 1]
        bottom = pseudo_boxes[..., 3] - ys
        ...
        return bbox_targets, gt_points, gt_labels, gt_bboxes_ref
