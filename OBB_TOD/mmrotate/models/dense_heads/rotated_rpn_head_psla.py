...
class PSRotatedRPNHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(PSRotatedRPNHead, self).__init__(
            1, in_channels, init_cfg=init_cfg, **kwargs)
        if self.train_cfg.assigner['type'] == 'TopkAssigner':
    ...
    def get_targets(self,
        ...
        if self.use_prediction:
            ...
            for i in range(num_imgs):
                temp_scores = []
                temp_preds = []
                for j in range(len(num_level_anchors)):
                    cls_scores_detach = cls_scores[j].clone().detach()
                    bbox_preds_detach = bbox_preds[j].clone().detach()

                    cls_scores_temp = cls_scores_detach.permute(0,2,3,1).reshape(num_imgs, -1, self.cls_out_channels)
                    bbox_preds_temp = bbox_preds_detach.permute(0,2,3,1).reshape(num_imgs, -1, 6)
                    
                    temp_scores.append(cls_scores_temp[i,:,:])
                    temp_preds.append(bbox_preds_temp[i,:,:])
                scores_anchors_before = torch.cat(temp_scores, dim=0)
                bboxes_anchors_before = torch.cat(temp_preds, dim=0)
                concat_cls_scores.append(scores_anchors_before)
                concat_bbox_preds.append(bboxes_anchors_before)

                gt_labels_list.append(torch.zeros(gt_bboxes_list[i].shape[0]).long().to(gt_bboxes_list[i].device))
