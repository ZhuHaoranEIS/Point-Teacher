...
def random_rescale_image(image_tensor, coordinates, gt_bboxes, scale_factor_uniform=[0.5,1.5], version='le90'):
    ...
    return tuple([output_image, coordinates_rescale, ignore_gt, scale_factor, gt_bboxes_rescale])
...
def fix_rescale_rbbox(image_tensor, gt_bboxes, insider, scale_factor_uniform, version='le90'):
    ...
    coordinates_rescale = gt_bboxes.clone()
    
    scale_factor = scale_factor_uniform
    ...
    if scale_factor < 1.0:
        blank_w = int((W - new_W)/2)
        blank_h = int((H - new_H)/2)
        coordinates_rescale[:,0] = (gt_bboxes[:,0] * scale_factor + blank_w).long()
        coordinates_rescale[:,1] = (gt_bboxes[:,1] * scale_factor + blank_h).long()
    else:
        blank_w = int((new_W - W)/2)
        blank_h = int((new_H - H)/2)
        coordinates_rescale[:,0] = (gt_bboxes[:,0] * scale_factor).long()
        coordinates_rescale[:,1] = (gt_bboxes[:,1] * scale_factor).long()
    ...
    return tuple([coordinates_rescale[pos_inds]])
...
def random_rotate_image(image_tensor, coordinates, gt_bboxes, angle_range=[1,360], version='le90'):
    ...
    for i in range(num_bboxes):
        if gt_bboxes != None:
            x1, y1, x2, y2, x3, y3, x4, y4 = bboxes_poly[i]
            ...
            bboxes_poly_rotated[i] = torch.tensor([x1_new, y1_new, x2_new, y2_new, x3_new, y3_new, x4_new, y4_new])

        x1, y1 = coordinates_poly[i]
        
        # Rotate each corner
        x1_new, y1_new = rotate_point(x1.item(), y1.item(), -angle, cx, cy)
        
        coordinates_poly_rotated[i] = torch.tensor([x1_new, y1_new])
        
        # Check if the rotated box is within the image
        if (0 <= x1_new < W and 0 <= y1_new < H):
...
def random_translate_image(image_tensor, coordinates, gt_bboxes, angle_range=[1,360], ratio_range=[0.1,0.5], version='le90'):
    ...
    for i in range(num_bboxes):
        ...
        coordinates_translated[i] = torch.tensor([cx_new, cy_new], dtype=coordinates.dtype, device=coordinates.device)
    ...
    if gt_bboxes == None:
        ...
        # Translate the boxes
        gt_bboxes_translate = torch.zeros_like(gt_bboxes)
        
        for i in range(num_bboxes):
            cx, cy, w, h, theta = gt_bboxes[i]
            
            # Translate the center of the box
            cx_new, cy_new = translate_point(cx.item(), cy.item(), dx, dy)
            gt_bboxes_translate[i] = torch.tensor([cx_new, cy_new, w, h, theta])
    '''
...
if __name__=='__main__':
    ...
    ### random translate
    # structed_tree_path = '/home/zhuhaoran/SODA/mmrotate/models/utils/image_operate/examples/st_test.png'
    # image_vec = cv2.imread(soda_image_path, cv2.COLOR_BGR2GRAY)
    # g_blurred = cv2.GaussianBlur(image_vec, (3, 3), 0)

    # blurred_float = g_blurred.astype(np.float32) / 255.0
    # import time; time1 = time.time()
    # edgeDetector = cv2.ximgproc.createStructuredEdgeDetection("/home/zhuhaoran/SODA/mmrotate/models/utils/model.yml")

    # edges = edgeDetector.detectEdges(blurred_float) * 255.0
    # time2 = time.time()
    # print(time2-time1)
    # cv2.imwrite(structed_tree_path, edges)

    # save_image(image, save_ori_image_path)
    # imshow_det_rbboxes(structed_tree_path,
    #                    bboxes=coordinates.numpy(),
    #                    labels=np.ones(len(coordinates)).astype(int),
    #                    bbox_color='red',
    #                    text_color='red',
    ...
    soda_image_path = '/data/zhr/SODA/SODA-A/divData/train/Images_filter/'
    image_name = '01687__800__3955___1300.jpg'
    soda_image_path = soda_image_path + image_name
    image = cv2.imread(soda_image_path, cv2.IMREAD_GRAYSCALE)

    # laplacian = cv2.Laplacian(image, cv2.CV_64F)
    # laplacian = cv2.convertScaleAbs(laplacian)
    # mean_val = np.mean(laplacian)
    # binary_mask = np.zeros_like(laplacian)
    # binary_mask[laplacian > mean_val] = 255

    # sobelx = cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=3)
    # sobely = cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=3)
    # sobel = np.hypot(sobelx, sobely)
    # mean_val = np.mean(sobel)
    # binary_mask = np.zeros_like(sobel)
    # binary_mask[sobel > mean_val] = 255

    # from scipy.ndimage import prewitt
    # prewittx = prewitt(image, axis=0)
    # prewitty = prewitt(image, axis=1)
    # prewitt_edges = np.hypot(prewittx, prewitty)
    # mean_val = np.mean(prewitt_edges)
    # binary_mask = np.zeros_like(prewitt_edges)
    # binary_mask[prewitt_edges > mean_val] = 255


    # scharrx = cv2.Scharr(image, cv2.CV_64F, 1, 0)
    # scharry = cv2.Scharr(image, cv2.CV_64F, 0, 1)
    # scharr = np.hypot(scharrx, scharry)
    # mean_val = np.mean(scharr)
    # binary_mask = np.zeros_like(scharr)
    # binary_mask[scharr > mean_val] = 255

    # import os
    # base_path = '/data/zhr/SODA/SODA-A/divData/train/Images_filter'
    # save_path = '/data/zhr/SODA/SODA-A/divData/train/Images_filter_canny'
    # file_all = os.listdir(base_path)
    # for i in range(len(file_all)):
    #     image_path = os.path.join(base_path, file_all[i])
    #     save_image_path = os.path.join(save_path, file_all[i])
    #     image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
    #     blurred = cv2.GaussianBlur(image, (5, 5), 0)
    #     edges = cv2.Canny(blurred, threshold1=100, threshold2=200)
    #     mean_val = np.mean(edges)
    #     binary_mask = np.zeros_like(edges)
    #     binary_mask[edges > mean_val] = 255
    #     cv2.imwrite(save_image_path, binary_mask)
    ...
    for kernel in kernels:
        ...
        edges = np.maximum(edges, response)
    ...
    # import pdb; pdb.set_trace()
    cv2.imwrite('/home/zhuhaoran/SODA/mmrotate/models/utils/image_operate/examples/st_test.png', binary_mask)
