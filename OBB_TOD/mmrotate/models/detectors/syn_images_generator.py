...
def generate_sythesis(img, bb_occupied, img_syn, sca_fact, pattern, prior_size,
    ...
    scale_vary = torch.rand(bb_occupied.shape[0]).to(device) + 0.5
...
if __name__ == '__main__':
    pattern, prior_size = load_basic_pattern(
        '/home/zhr/SODA/mmrotate/models/detectors/basic_patterns/basic_bank')

    img = cv2.imread('/data2/zhr/SODA/SODA-A/divData/train/Images_filter/00001__800__3900___0.jpg')
    ...
    # bb1[:, 5] = 1
    c = time.time()
    img, bb2 = generate_sythesis(img, bb, img, 1.0, pattern, prior_size, 
                                 range(len(pattern)), 800, scale_ratio=1.0)
    # import pdb; pdb.set_trace()
    print(time.time() - c)
    img = img.permute(1, 2, 0).contiguous().cpu().numpy()

    cv2.imwrite('/home/zhr/SODA/mmrotate/models/detectors/vis_points/P2RB_sys.png', img)

    coordinates_flip = bb2[:,:5]
    imshow_det_rbboxes('/home/zhr/SODA/mmrotate/models/detectors/vis_points/P2RB_sys.png',
                       bboxes=coordinates_flip.numpy(),
                       labels=np.ones(len(coordinates_flip)).astype(int),
                       bbox_color='red',
                       text_color='red',
                       out_file='/home/zhr/SODA/mmrotate/models/detectors/vis_points/P2RB_sys.png')
